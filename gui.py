...
class GestureEditor:
    ...
    def create_widgets(self):
        ...
        # Right Frame
        self.radio_frame = ttk.LabelFrame(self.right_frame, text="Gestures", padding=(20, 10))
        self.radio_frame.pack(fill="both", expand=True, padx=10, pady=10)
        
        self.radio_buttons = []
        for gesture in self.video_paths.keys():
            radio_button = ttk.Radiobutton(self.radio_frame, text=gesture, variable=self.selected_gesture, value=gesture, command=self.update_preview)
        ...
        ttk.Button(self.bottom_frame, text="New", command=self.add_new_gesture).pack(side="left", padx=5)
        ttk.Button(self.bottom_frame, text="Delete", command=self.delete_gesture).pack(side="left", padx=5)
        ttk.Button(self.bottom_frame, text="Save", command=self.save_gestures).pack(side="right", padx=5)
        ttk.Button(self.bottom_frame, text="Cancel", command=self.master.destroy).pack(side="right", padx=5)
    ...
    def add_new_gesture(self):
        ...
        save_button = ttk.Button(popup, text="Save", command=lambda: self.save_new_gesture(popup, video_dropdown.get()))
    ...
    def save_new_gesture(self, popup, video_name):
        ...
        radio_button = ttk.Radiobutton(self.radio_frame, text=gesture_name, variable=self.selected_gesture, value=gesture_name, command=self.update_preview)
        radio_button.pack(anchor="w", padx=5, pady=5)
        self.radio_buttons.append(radio_button)
        popup.destroy()

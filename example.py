...
# Import the tcl file
root.tk.call("source", r"C:\Users\medha\Desktop\Forest-ttk-theme-master\Forest-ttk-theme-master\forest-dark.tcl")
...
def open_edit_gesture_popup():
    edit_gesture_popup = tk.Toplevel(root)
    edit_gesture_popup.title("Edit Gesture")
    
    # Create widgets for edit gesture
    key_bind_label = tk.Label(edit_gesture_popup, text="Key Bind:")
    key_bind_label.pack(pady=5)
    
    # Entry for key bind
    key_bind_entry = tk.Entry(edit_gesture_popup)
    key_bind_entry.pack(pady=5)
    
    # Button to preview
    preview_button = tk.Button(edit_gesture_popup, text="Preview")
    preview_button.pack(pady=5)

def open_new_gesture_popup():
    new_gesture_popup = tk.Toplevel(root)
    new_gesture_popup.title("New Gesture")
    
    # Create widgets for new gesture
    new_gesture_label = tk.Label(new_gesture_popup, text="New Gesture Options")
    new_gesture_label.pack(pady=5)
    
    # Additional options for new gesture
    # You can add more widgets as needed
    
# Create the main application window
root = tk.Tk()
root.title("Gesture Management")

# Create frame to hold buttons
button_frame = ttk.Frame(root)
button_frame.pack(pady=20)

# Button to edit gesture
edit_gesture_button = ttk.Button(button_frame, text="Edit Gesture", command=open_edit_gesture_popup)
edit_gesture_button.grid(row=0, column=0, padx=10)

# Button to create new gesture
new_gesture_button = ttk.Button(button_frame, text="New Gesture", command=open_new_gesture_popup)
new_gesture_button.grid(row=0, column=1, padx=10)
